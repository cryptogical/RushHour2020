...
int main() {
    printf("\n\n");
    printf("				RUSH HOUR XTREME\n\n\n");	
    printf("Numéro de la carte de jeu?\n");
    int n;
    char c;
	scanf("%d",&n);
	printf("\n");
	dlconfig(n);
    grille();
    printf("[J]ouer ou [A]utosolving? ");
    scanf("%c",c);
    printf("\n");
    if (strcmp(c,j)==0) {
        while(true) {
            printf("Choisissez le déplacement sous cette forme : Couleur Direction NbrCase \n");
            printf("Couleurs: ");
            for (int i = 0; i < 16; ++i)
            {
                printf("%s ", listecouleurs[i]);
            }
            printf("\n");
            if(ended()) {
                break;
